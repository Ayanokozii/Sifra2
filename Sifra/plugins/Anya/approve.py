...
# Welcome message to be sent to the user after approving their join request
...
async def accept_join_request(client: Client, chat_request: ChatJoinRequest):
    try:
        ...
        # Send the welcome message as a private message
...
# Handler for broadcasting a message to all users via personal messages
@app.on_message(filters.command("broadcast") & filters.user(6999372290))  # Replace with your Telegram user ID
async def broadcast_message(client: Client, message):
    ...
    try:
        # Broadcast the message to all members in the chat via private messages
